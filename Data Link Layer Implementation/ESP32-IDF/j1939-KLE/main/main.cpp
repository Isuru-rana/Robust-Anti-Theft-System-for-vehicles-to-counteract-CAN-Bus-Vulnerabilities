 ...
 * @brief J1939 CAN Bus KeyLess Entry Module (KLE) with SMS Notification for ESP32
 * @version 1.0
 * 
 * This application implements a KeyLess Entry Module (KLE) for J1939 protocol
 * communication over CAN bus with GSM/SMS notification capabilities using the ESP32.
 * 
 * Features:
 * - Bidirectional communication between UART and J1939 CAN bus
 * - GSM module integration for SMS notifications
 * - Support for both JSON-formatted commands and direct CAN message transmission
 * - Message queuing system for both CAN and SMS messages
 ...
 * - GSM module connected via UART1
 *   - TX=GPIO17, RX=GPIO16
 * - CAN communication on UART0
 * 
 * The program processes two types of inputs via UART0:
 * 1. JSON messages: Format {"c":"command","d":"data"}
 *    - Command "np" with any data triggers an SMS message
 *    - SMS content is the data value from the JSON
 * 
 ...
 * 
 * SMS notifications are sent to a predefined phone number when triggered by JSON commands.
 * 
 */
...
#define UART_GSM_NUM UART_NUM_1
#define BUF_SIZE 1024

#define GSM_TX_PIN 17
#define GSM_RX_PIN 16

#define DEFAULT_PHONE_NUMBER "+94716074350"
...
void init_gsm();
bool wait_for_gsm_response(const char* expected_response, uint32_t timeout_ms);
...
bool process_json_message(const uint8_t *data, size_t len) {
    ...
    if (is_valid) {
        ...
        if (strcmp(cmd, "np") == 0) {
            ...
            sms_task_message_t sms_msg;
            strncpy(sms_msg.phone_number, DEFAULT_PHONE_NUMBER, sizeof(sms_msg.phone_number) - 1);
            sms_msg.phone_number[sizeof(sms_msg.phone_number) - 1] = '\0';
            
            strncpy(sms_msg.message, data_val, sizeof(sms_msg.message) - 1);
            sms_msg.message[sizeof(sms_msg.message) - 1] = '\0';
            
            if (xQueueSend(sms_queue, &sms_msg, pdMS_TO_TICKS(1000)) != pdPASS) {
                // ESP_LOGE(TAG, "Failed to queue SMS message");
            } else {
                // ESP_LOGI(TAG, "SMS message queued successfully");
            }
        }
    }
...
void init_gsm() {
    ...
    ESP_ERROR_CHECK(uart_param_config(UART_GSM_NUM, &uart_config));
    
    ESP_ERROR_CHECK(uart_set_pin(UART_GSM_NUM, GSM_TX_PIN, GSM_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    
    ESP_ERROR_CHECK(uart_driver_install(UART_GSM_NUM, BUF_SIZE * 2, BUF_SIZE * 2, 0, NULL, 0));
    ...
    gsm_send_command("AT\r\n");
    if (wait_for_gsm_response("OK", 1000)) {
        // ESP_LOGI(TAG, "GSM module responded to AT command");
    } else {
        // ESP_LOGW(TAG, "GSM module did not respond to AT command");
    }
    
    gsm_send_command("AT+CMGF=1\r\n");
    wait_for_gsm_response("OK", 1000);
    
    // ESP_LOGI(TAG, "GSM module initialization completed");
}

void gsm_send_command(const char* command) {
    uart_write_bytes(UART_GSM_NUM, command, strlen(command));
    // ESP_LOGI(TAG, "GSM command sent: %s", command);
}
...
bool wait_for_gsm_response(const char* expected_response, uint32_t timeout_ms) {
    ...
    while ((esp_log_timestamp() - start_time) < timeout_ms) {
        uint8_t c;
        int len = uart_read_bytes(UART_GSM_NUM, &c, 1, 20 / portTICK_PERIOD_MS);
        if (len > 0) {
            if (position < sizeof(response) - 1) {
                response[position++] = c;
                response[position] = '\0';
                
                if (strstr(response, expected_response) != NULL) {
                    // ESP_LOGI(TAG, "GSM response received: %s", response);
                    return true;
                }
            }
        }
    ...
    // ESP_LOGW(TAG, "GSM response timeout. Last received: %s", response);
    return false;
}
...
void sms_task(void *pvParameters) {
    ...
    // ESP_LOGI(TAG, "SMS task started");
    
    while (1) {
        if (xQueueReceive(sms_queue, &sms_msg, portMAX_DELAY)) {
            // ESP_LOGI(TAG, "SMS message received in queue: %s", sms_msg.message);
            ...
            char command[128];
            snprintf(command, sizeof(command), "AT+CMGS=\"%s\"\r\n", sms_msg.phone_number);
            gsm_send_command(command);
            if (!wait_for_gsm_response(">", 1000)) {
                // ESP_LOGE(TAG, "Failed to set phone number");
                continue;
            }
            
            uart_write_bytes(UART_GSM_NUM, sms_msg.message, strlen(sms_msg.message));
            
            uint8_t ctrlZ = 26;
            uart_write_bytes(UART_GSM_NUM, (const char*)&ctrlZ, 1);
            
            if (!wait_for_gsm_response("+CMGS:", 10000)) {
...
void sender_task(void *pvParameters) {
    ...
    uart_driver_install(UART_CAN_NUM, BUF_SIZE * 2, 0, 0, NULL, 0);
    ...
    while (1) {
        ...
        int len = uart_read_bytes(UART_CAN_NUM, data_ptr, 1, message_queue.empty() ? portMAX_DELAY : 10);
        if (len > 0) {
            data_ptr++;
            data_len++;
            if (*(data_ptr - 1) == '\n' || *(data_ptr - 1) == '\r' || data_len >= BUF_SIZE - 1) {
                *data_ptr = '\0';
                ...
                if (process_json_message(data, data_len)) {
                    // Don't echo JSON messages back to serial
                    // ESP_LOGI(TAG, "JSON message processed but not echoed back or sent to CAN bus");
                } else {
                    // Don't echo non-JSON messages either
...
extern "C" void app_main(void) {
    ...
    sms_queue = xQueueCreate(10, sizeof(sms_task_message_t));
    if (sms_queue == NULL) {
    ...
    xTaskCreate(sms_task, "sms_task", 4096, NULL, 5, &sms_task_handle);
