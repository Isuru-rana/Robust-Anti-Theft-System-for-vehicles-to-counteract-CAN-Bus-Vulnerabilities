 ...
 * @brief J1939 CAN Bus Command Line Manager for ESP32
 * @version 1.0
 * 
 * This application implements a Command Line Manager (CLM) for J1939 protocol
 ...
 * - GPIO control for status indication and output management
 ...
 * LED outouts (for the door locks):
 * - Managed output pins: GPIO2(built-in LED), GPIO15, GPIO4, GPIO22
 * 
 * The program processes two types of inputs via UART:
 * 1. JSON messages: Format {"c":"command","d":"data"} for GPIO control
 *    - Command "np" with data "ON"/"unlock" turns GPIO pins on
 *    - Command "np" with data "OFF"/"lock" turns GPIO pins off
 *    - Other commands/data trigger temporary GPIO activation
 * 
...
#define GPIO_PIN_15 GPIO_NUM_15
#define GPIO_PIN_4 GPIO_NUM_4
#define GPIO_PIN_22 GPIO_NUM_22

const gpio_num_t managed_pins[] = {BUILTIN_LED, GPIO_PIN_15, GPIO_PIN_4, GPIO_PIN_22};
#define NUM_MANAGED_PINS (sizeof(managed_pins) / sizeof(managed_pins[0]))
...
bool process_json_message(const uint8_t *data, size_t len) {
    ...
    if (is_valid) {
        ...
        if (strcmp(cmd, "np") == 0) {
            ...
            if (strcmp(data_val, "ON") == 0 || strcmp(data_val, "unlock") == 0) {
                // ESP_LOGI(TAG, "Turning all GPIOs permanently ON");
                led_msg.turn_on = true;
                led_msg.duration_ms = 0;
                xQueueSend(led_control_queue, &led_msg, portMAX_DELAY);
            } 
            else if (strcmp(data_val, "OFF") == 0 || strcmp(data_val, "lock") == 0) {
                // ESP_LOGI(TAG, "Turning all GPIOs permanently OFF");
                led_msg.turn_on = false;
                led_msg.duration_ms = 0;
                xQueueSend(led_control_queue, &led_msg, portMAX_DELAY);
            }
            else {
                // ESP_LOGI(TAG, "Other 'np' command, blinking LEDs");
                led_msg.turn_on = true;
            ...
            // ESP_LOGI(TAG, "Command '%s' detected, blinking LEDs", cmd);
            led_control_t led_msg;
            led_msg.turn_on = true;
            led_msg.duration_ms = 2000;
            xQueueSend(led_control_queue, &led_msg, portMAX_DELAY);
...
void init_gpio_pins() {
    ...
    for (int i = 0; i < NUM_MANAGED_PINS; i++) {
        io_conf.pin_bit_mask = (1ULL << managed_pins[i]);
        gpio_config(&io_conf);
        
        gpio_set_level(managed_pins[i], 0);
        // ESP_LOGI(TAG, "GPIO pin %d initialized", managed_pins[i]);
    }
}
        ...
        if (xQueueReceive(led_control_queue, &control_msg, portMAX_DELAY)) {
            if (control_msg.turn_on) {
                if (control_msg.duration_ms > 0) {
                    // ESP_LOGI(TAG, "Turning on all GPIOs for %" PRIu32 " ms", control_msg.duration_ms);
                    for (int i = 0; i < NUM_MANAGED_PINS; i++) {
                        gpio_set_level(managed_pins[i], 1);
                    }
                    vTaskDelay(control_msg.duration_ms / portTICK_PERIOD_MS);
                    for (int i = 0; i < NUM_MANAGED_PINS; i++) {
                        gpio_set_level(managed_pins[i], 0);
                    }
                    // ESP_LOGI(TAG, "All GPIOs turned off");
                } else {
                    // ESP_LOGI(TAG, "Turning on all GPIOs permanently");
                    for (int i = 0; i < NUM_MANAGED_PINS; i++) {
                        gpio_set_level(managed_pins[i], 1);
                    }
                }
            } else {
                // ESP_LOGI(TAG, "Turning off all GPIOs");
                for (int i = 0; i < NUM_MANAGED_PINS; i++) {
                    gpio_set_level(managed_pins[i], 0);
                }
            }
            ...
            if (*(data_ptr - 1) == '\n' || *(data_ptr - 1) == '\r' || data_len >= BUF_SIZE - 1) {
                *data_ptr = '\0';
                ...
                if (process_json_message(data, data_len)) {
                    // JSON messages processed but not echoed back
                    // ESP_LOGI(TAG, "JSON message not sent to CAN bus");
                } else {
                    // Process as J1939 message
